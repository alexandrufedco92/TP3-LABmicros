...
#define	PDB_LOAD_VALUE_DEF			0		//valores por default para el pdb y su conexion con el dac
...
void PDBchangeFrequency(uint32_t new_freq){
	...
	setModulusValue((uint32_t)(value));		//Cargo valores  
	setCounterDelayValue((uint32_t)(value));
	PDBsoftwareTrigger();				//y triggereo el pdb
}

void setModulusValue(uint32_t value){		//cargo nueva frecuencia
	PDB0->MOD = PDB_MOD_MOD(value);
}

void setCounterDelayValue(uint32_t value){	//nueva frecuencia para el dac
...
void getPDBdefaultConfig(pdb_config_t* conf){		//seteo valores por default
	...
	conf->trigger_input_source = PDB_TRIG_INPUT_DEF;//elijo software trigger
...
void getPDBforDACdefaultConfig(pdb_dac_config_t* conf){		//confiugracion por default para la conexion con el pdb que trigerea dac
