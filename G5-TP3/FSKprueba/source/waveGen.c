...
void pwmSinWaveGen(WAVEGENid id, WAVEGENfreq freq)
{
	...
	initDMA();
	conf.source_address = (uint32_t)pwmSenLUT;		//de la tabla de valores del duty cycle
	conf.dest_address = (uint32_t)getCnVadress(FTM0_INDEX, FTM_CH0);		//al registro del CnV
	conf.offset = 0x02;			
	conf.transf_size = BITS_16;		//transfiero 16 bits por minor loop
	conf.bytes_per_request = 0x02;	//paso 16bits=2bytes en cada dma request
	conf.total_bytes = conf.bytes_per_request*N_SAMPLES;	//el total será 2bytes*cantidad_de_samples
	conf.mode = MEM_2_PERIPHERAL;		//de memoria a periferico
	conf.channel = DMA_WAVEGEN_CH;
	conf.dma_callback = NULL;			
	conf.periodic_trigger = true;		//triggereado periodicamente por PIT
	conf.request_source = DMA_PIT1;


//configuracion para channel linking, no utilizada
...
//	confTrigger.request_source = 60;
	...
	PITstartTimer(&configP0);			//PIT0 que triggerea dma request
	uint8_t i=0;
	for(i=0;i<4;i++)					//delay para el PIT1 que hace el software trigger para el ftm
	{
		asm("nop");
	}
